...
import { Search, Filter, Eye, Package, Truck, CreditCard } from 'lucide-react';
          ...
          >
            <Card className="card-hover">
              <CardContent className="p-6">
                ...
                <div className="mt-2">
                    <p className="text-xs text-gray-500">Shipping Address: <span className="font-medium text-gray-700">{order.address}</span></p>
                </div>
              </CardContent>
            </Card>
          </motion.div>
        ...
        {statusOptions.map(status => (
             <Card key={status}>
                ...
                <p className={`text-xl font-bold ${getStatusColor(status).split(' ')[1]}`}>
                    {orders.filter(o => o.status === status || (status === 'all' && o.status)).length}
                </p>
